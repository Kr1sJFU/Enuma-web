...
// Exact 32 fps boundaries in the current 30.875-second master.
const chapters = [
  { start: 0, end: 8.0625, label: 'Press the button', action: 'ACTIVATE', key: 'E', hotspot: 'is-button' },
  { start: 8.0625, end: 17.4375, label: 'Enter the mirror', action: 'FORWARD', key: 'W', hotspot: 'is-mirror' },
  { start: 17.4375, end: 22.8125, label: 'Cross the stars', action: 'FORWARD', key: 'W', hotspot: 'is-star' },
  { start: 22.8125, end: 30.875, label: 'Ignite the path', action: 'IGNITE', key: 'E', hotspot: 'is-rune' },
];
...
function render() {
  ...
  stepNumber.textContent = `${String(chapterIndex + 1).padStart(2, '0')} / 04`;
...
function reset() {
  ...
  mode = 'waiting';
  render();
  announcer.textContent = 'Restarted. Press E or use the action button.';
}
...
document.addEventListener('keydown', event => {
  ...
  if (event.repeat || ['BUTTON', 'A'].includes(document.activeElement?.tagName)) return;
