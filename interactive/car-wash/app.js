...
function render() {
  ...
  stepNumber.textContent = `${String(chapterIndex + 1).padStart(2, '0')} / ${String(chapters.length).padStart(2, '0')}`;
...
function reset() {
  ...
  mode = 'waiting';
  render();
  announcer.textContent = `Restarted. Press ${chapters[0].key} or use the action button.`;
}
